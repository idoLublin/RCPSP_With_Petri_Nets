...
// ---- init-F computation: root node f = root_makespan + h(root), NO search ----
// Replicates the root-state setup of solveRCPSP_CBS_impl<N> and reports the
// initial lower bound on the makespan for every instance under a given config.
template<int N>
void computeInitF_impl(const std::string& problemType, int configNum, std::ofstream& file) {
    for (int group = 1; group <= 48; group++) {
        for (int exam = 1; exam <= 10; exam++) {
            ...
            RCPSPState_CBS<N> first;                              // root state (earliest-start schedule)
            int root_mk = (int)first.start_times[g_sink_id];     // root makespan (CPM-style LB)
            int h_root  = (int)first.compute_h_and_RVS();         // root heuristic (config-dependent)
            long init_F = (long)root_mk + (long)h_root;           // initial makespan lower bound
            file << group << "," << exam << "," << configNum << ","
                 << root_mk << "," << h_root << "," << init_F << "\n";
            file.flush();
        }
    }
}

void computeInitF(const std::string& problemType, int configNum) {
    setProblemSize(problemType);
    applyConfigNum(configNum);
    std::string fn = "initF_" + problemType + "_cfg" + std::to_string(configNum) + ".csv";
    std::ofstream file(fn);
    file << "group,exam,config,root_makespan,h_root,init_F\n";
    if (problemType == "j30")      computeInitF_impl<32>(problemType, configNum, file);
    else if (problemType == "j60") computeInitF_impl<62>(problemType, configNum, file);
    else if (problemType == "j90") computeInitF_impl<92>(problemType, configNum, file);
    else std::cerr << "unknown size: " << problemType << "\n";
    file.close();
...
int main(int argc, char* argv[]) {
    ...
    if (argc >= 2) {
        ...
        if (arg1 == "nmd_test") {
            ...
            // init-F mode: Driver initf <size> <cfg>  -> writes initF_<size>_cfg<cfg>.csv
            if (argc < 4) { std::cerr << "Usage: initf <size> <cfg>\n"; return 1; }
            computeInitF(argv[2], std::atoi(argv[3]));
        } else if (argc >= 3) {
