...
bool useCS;
// Heuristic selector: "cp" (critical path), "cc" (critical capacity), etc.
std::string activeHeuristic = "cp";
...
//   }
// }
// std::vector<Transition> getAvilableTransitions(std::map<std::string, int>
// marking);
std::vector<P_RCPSP::Transition>
getAvilableTransitions(const std::unordered_map<std::string, int> &marking);
double computeWorkloadLowerBoundWithMax(
    ...
    const std::map<int, int> &earlyStartTimes, double criticalPathEstimate);
...
std::vector<int> getCriticalPath(
    const std::map<int, int> &earlyStartTimes, int lastActivityId,
...
double computeSequenceLowerBoundWithMax2(
    const std::vector<short> &unfinishedTransitions,
    const std::vector<std::pair<short, short>> &activeTransitionIndices,
    std::map<int, int> &earlyStartTimes, std::map<int, int> &earlyfinishTimes,
    double criticalPathEstimate, std::map<int, int> finishedActivities);
...
int main2() { return 0; }
// std::vector<Transition> getAvilableDetransitions(const
...
std::vector<int> getCriticalPath(const std::map<int, int> &earlyfinishTimes,
  ...
  while (current != -1) {
    ...
    if (current < 1 ||
        current > static_cast<int>(RCPSPex.backword_dependencies.size())) {
      break;
    }

    const auto &deps =
        RCPSPex.backword_dependencies[current - 1]; // Convert to 0-based
...
//         const auto& successors = RCPSPex.dependencies[i - 1]; // Convert to
//         0-based
//
...
//
//             // Check if this successor already has a late finish time
//             calculated auto it = latefinishTimes.find(succId); if (it !=
//             latefinishTimes.end()) {
...
//             duration = RCPSPex.activities[i-1].duration; // Convert to
//             0-based
//         }
//
//         // Calculate late finish time: min(successor late finish times) -
//         duration latefinishTimes[i] = minLateFinish - duration;
//     }
//
...
double computeSequenceLowerBoundWithMax2(
  ...
  std::vector<int> path =
      getCriticalPath(earlyfinishTimes, RCPSPex.activities.size());
  std::map<int, int>
      latestartTimes; // getlatefinish(RCPSPex.activities.size(),criticalPathEstimate);

  path.erase(std::remove_if(path.begin(), path.end(),
                            [&finishedActivities](int activityId) {
                              return finishedActivities.find(activityId) !=
                                     finishedActivities.end();
                            }),
             path.end());
  ...
  for (const auto &actId : path) {
    ...
    for (int t = earlyStartTimes[actId];
         t < act.duration + earlyStartTimes[actId]; t++) {
...
double computeCoreTimeLowerBoundWithMax(
  ...
  int high =
      static_cast<int>(low + std::ceil(totalWork / std::max(1, minCapacity)));
  ...
  while (low <= high) {
    ...
    for (int id : unstartedTransitions) {
      ...
      // Proper core interval: where the activity *must* overlap if makespan is
      // mid
...
double computeEarliestFinish(int activityId,
                             ...
                             const std::map<int, int> &finishedActivities) {
  // Memoization check
  if (earlyFinishMemo.count(activityId))
    return earlyFinishMemo[activityId];
  ...
  //     if (std::find(unstartedTransitions.begin(), unstartedTransitions.end(),
  //     depId) != unstartedTransitions.end()) {
  //         double depFinish = computeEarliestFinish(depId, earlyFinishMemo,
  //         unstartedTransitions, finishedActivities)
  //                          + RCPSPex.activities[depInternal].duration;
  //         maxFinish = std::max(maxFinish, depFinish);
  //     } else {
  //         // If dependency is finished, consider its finish time as stored in
  //         earlyFinishMemo maxFinish = std::max(maxFinish,
  //         earlyFinishMemo[depId]);
  //     }
  // }
